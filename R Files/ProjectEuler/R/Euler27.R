#' Euler 27
#'
#' Performs a coefficient search to see how many consecutive primes can be produced from the
#' equation n^2 + a*n + b
#'
#' @param verbose If true will print a message
#'
#' @return Integer
#' @export
euler27 <- function(verbose = F) {
	...
	for(bValue in possibleB) {
		for (aValue in possibleA) {
			...
			if(consecutive - 1 > max.consecutive) {
				...
				# Print message
				if (verbose) print(paste0("The longest sequence is ",max.consecutive, " from a = ", maxA," and b = ", maxB))
			}
		}
	}
	maxA*maxB
